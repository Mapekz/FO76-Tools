...
| `src/diff.rs` | `diff_databases_with(a,b,opts)` — byte-equality fast-path, sparse `{from,to}` JSON diff; per-element array diffs (`_array_diff`) in one of four strategies — `keyed`/`positional`/`set`/`unkeyed` (`element_key_spec`'s twelve heuristics *propose* element identity — the sole owner, see `docs/adr/0005-element-identity-owned-by-rust.md` — but `widen_key_spec_until_unique` then validates the proposal against the actual pair of arrays, appending further scalar fields until unique on both sides or falling back to `unkeyed` if none does, so a key is never assumed unique from the sample shape alone; `unkeyed` is a deliberate classification for arrays with no stable per-element identity, e.g. CTDA `Conditions[]`, not a fallback of last resort — `removed`/`added` are the elements outside an order-preserving LCS alignment (`lcs_align`) across the two sides, plus `unchanged_count`, never a bare `{from,to}` leaf or a full whole-list dump); `DiffOptions` (bodies for all added/removed records, placement/CELL noise suppression, type exclusion); `ref_names` sidecar |
