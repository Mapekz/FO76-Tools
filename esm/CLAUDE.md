...
| `src/walk.rs` | BFS over `ChaseFetcher` (same seam as `chase.rs`) printing one compact indented per-record-type digest (GLOB/AVIF/KYWD/MGEF/SPEL·ENCH·ALCH/PERK/WEAP/OMOD, generic fallback) instead of a series of raw `get` dumps; `build_refs_digest` groups a `--refs` reverse-reference summary by record type |
