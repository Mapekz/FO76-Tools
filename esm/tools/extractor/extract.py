...
# pascal_stubs` resolves regardless of how extract.py was invoked (direct
# script run, `from extract import ...` after audit.py's own sys.path
# bootstrap, or `from extractor.extract import ...` as an implicit namespace
# package from tools/tests/) — same pattern audit.py uses to import extract.
sys.path.insert(0, str(Path(__file__).resolve().parent))
import pascal_stubs  # noqa: E402
...
_WEAPON_PROPERTIES: list[str] = [
    ...
    "HasRepeatableSingleFire", "FullPowerDamageMult", "ColorRemappingIndex", "MaterialSwaps",
...
class Extractor:
    ...
    def _inject_builtin_helpers(self) -> None:
        """Seed self.vars with the hand-reverse-engineered Pascal-equivalent
        expansions for bare wbXxx helper variables the static parser can't
        resolve from wbDefinitionsFO76.pas / wbDefinitionsCommon.pas.

        The data lives in pascal_stubs.BUILTIN_HELPERS (unconditional —
        overwrites any auto-collected Pascal `:=` assignment, same as the
        original `self.vars[name] = value` calls this replaced) and
        pascal_stubs.BUILTIN_HELPERS_DEFAULTS (only fills a name that
        _collect_vars didn't already populate, same as the original
        `self.vars.setdefault(name, value)` calls) — see that module's
        docstring for why each entry exists.
        """
        for _name, _value in pascal_stubs.BUILTIN_HELPERS.items():
            self.vars[_name] = _value
        for _name, _value in pascal_stubs.BUILTIN_HELPERS_DEFAULTS.items():
            self.vars.setdefault(_name, _value)
    ...
    def expand_call(self, expr: str) -> str:
        ...
        if m:
            ...
            # ~21 named Pascal helper-call expansions live as data in
            # pascal_stubs.CALL_EXPANSIONS (see its module docstring): a table
            # value is either a fixed template string (args are irrelevant to
            # the expansion) or a (args, ExpandContext) -> str | None
            # callable that substitutes the parsed args into a template.
            # Returning None means "no expansion, fall through to the
            # self.vars/raw-expr default below" — the same behaviour an
            # original `if fn == "...":` branch had when it had no covering
            # `else` (e.g. wbSizePosRot with an unrecognized sig).
            handler = pascal_stubs.CALL_EXPANSIONS.get(fn)
            if handler is not None:
                if isinstance(handler, str):
                    return handler
                ctx = pascal_stubs.ExpandContext(
                    vars=self.vars,
                    raw_expr=expr,
                    split_top_level=split_top_level,
                    unquote=unquote,
                    sig_id=sig_id,
                    find_matching_bracket=find_matching_bracket,
                )
            ...
            #
            # Kept coded here rather than in pascal_stubs.CALL_EXPANSIONS: it caches
            # a pre-built schema dict into self._inline_members/_inline_counter,
            # per-Extractor-instance state the table's plain
            # (args, ExpandContext) -> str | None contract has no way to carry.
