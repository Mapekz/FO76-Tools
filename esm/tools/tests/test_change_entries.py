#!/usr/bin/env python3
"""Tests for tools/change_entries.py."""
...
class TestClassifyCut(unittest.TestCase):
    def test_zzz_prefix_high_confidence(self):
        ci = change_entries.classify_cut("zzz_OldPerk", prev_edid="OldPerk")
    ...
    def test_still_cut_when_prev_also_marked(self):
        ci = change_entries.classify_cut("zzz_OldPerk02", prev_edid="zzz_OldPerk01")
        self.assertEqual(ci["kind"], "still_cut")

    def test_added_cut_when_no_prev(self):
        ci = change_entries.classify_cut("CUT_PlaceholderThing")
    ...
    def test_poster_false_positive_guarded(self):
        ...
        self.assertIsNone(change_entries.classify_cut("Poster01"))
        self.assertIsNone(change_entries.classify_cut("PosterFrame"))

    def test_post_prefix_still_detected_with_delimiter(self):
        ci = change_entries.classify_cut("POST_FutureQuest")
    ...
    def test_post_bare_prefix_before_camelcase(self):
        ...
        ci = change_entries.classify_cut("POSTUpdateItem")
        self.assertIsNotNone(ci)
        self.assertEqual(ci["marker"], "POST")
        self.assertEqual(ci["confidence"], "low")

    def test_suffix_marker_low_confidence(self):
        ci = change_entries.classify_cut("SomeQuest_DELETE")
        self.assertIsNotNone(ci)
    ...
    def test_lowercase_bare_prefix_before_camelcase_medium_confidence(self):
        ...
        ci = change_entries.classify_cut("zzzLegendary_Weapon_PyromaniacPerk", prev_edid="Legendary_Weapon_PyromaniacPerk")
        self.assertIsNotNone(ci)
        self.assertEqual(ci["marker"], "ZZZ")
    ...
    def test_annotate_cut_on_bare_stub(self):
        ...
        ci = change_entries.annotate_cut(stub)
        self.assertEqual(ci["kind"], "added_cut")

    def test_annotate_cut_none_when_clean(self):
        stub = {"editor_id": "AssaultRifle", "form_id": "0x01"}
        self.assertIsNone(change_entries.annotate_cut(stub))
...
class TestExtractChangesKinds(unittest.TestCase):
    ...
    def _entries(self, form_id):
        ...
        return change_entries.extract_changes(rec["field_changes"], self.ref_names)
    ...
    def test_exhaustive_nothing_dropped(self):
        ...
        raw_entries = change_entries.extract_changes(
            {"Unknown Blob": {"from": {"_raw": True, "hex": "aa"}, "to": {"_raw": True, "hex": "bb"}}},
            {},
        )
...
class TestStructDisplay(unittest.TestCase):
    def test_dict_values_render_via_format_scalar_not_dropped(self):
        ...
        out = change_entries._struct_display(elem, ref_names)
    ...
    def test_list_and_null_values_skipped(self):
        ...
        out = change_entries._struct_display(elem, {})
    ...
    def test_flattens_nested_struct_member_lacking_its_own_display(self):
        # A RACE `Attacks` entry (unkeyed after key-widening exhausts its
        # candidates — `Required Slot` alone is shared by several attacks)
        # is exactly `{"Condition Data": {...}, "Parameter #1": ...}`: two
        # top-level keys, so `_unwrap_element_wrapper` correctly leaves it
        # alone (its unwrap only fires for single-key wrappers). Before
        # flattening, `Condition Data` had no dedicated rendering (no name/
        # formid/curve/flags) and fell to format_scalar's
        # `` `(struct: Operator, AND/OR, ...)` `` — as unhelpful as the
        # wrapper case `test_unwraps_ctda_condition_wrapper_to_real_fields`
        # covers. Its fields should splice into the one-liner directly,
        # alongside the sibling scalar `Parameter #1`.
        elem = {
            "Condition Data": {
                "Operator": "Equal To",
                "AND/OR": "AND",
                "Comparison Value": 1.0,
                "Function": "IsPreviousMeleeAttackEvent",
            },
            "Parameter #1": "meleeAttackStartForward_A",
        }
        out = change_entries._struct_display(elem, {})
        self.assertIn("Operator=`Equal To`", out)
        self.assertIn("AND/OR=`AND`", out)
        self.assertIn("Comparison Value=`1.0`", out)
        self.assertIn("Function=`IsPreviousMeleeAttackEvent`", out)
        self.assertIn("Parameter #1=`meleeAttackStartForward_A`", out)
        self.assertNotIn("(struct:", out)
        self.assertNotIn("Condition Data=", out)

    def test_flattening_does_not_recurse_past_one_level(self):
        # A struct nested inside the flattened member still falls back to
        # format_scalar's own `(struct: ...)` — flattening is bounded to
        # one level, matching `_unwrap_element_wrapper`'s own bound. Two
        # top-level keys ("Condition Data", "Parameter #1") so
        # `_unwrap_element_wrapper` (single-key-only) doesn't fire first and
        # add an extra, unrelated level of unwrapping ahead of the loop.
        elem = {
            "Condition Data": {
                "Inner": {"A": 1, "B": 2},
                "Sibling": 3,
        ...
        out = change_entries._struct_display(elem, {})
        self.assertIn("Sibling=`3`", out)
        self.assertIn("Inner=`(struct: A, B)`", out)
        self.assertIn("Parameter #1=`foo`", out)

    def test_flat_renderable_dict_member_still_uses_format_scalar(self):
        # A dict member WITH its own dedicated rendering (a resolved FormID
        # stub — formid + editor_id/record_type inline, the shape
        # `format_scalar`'s own dict branch and `_is_flat_renderable_dict`
        # both key on) must render through `annotate_ref`, not get
        # flattened into its formid/editor_id/record_type members.
        elem = {
            "Attack Spell": {
                "formid": "0x00000099",
                "record_type": "WEAP",
        ...
        out = change_entries._struct_display(elem, {})
        self.assertIn("Attack Spell=", out)
        self.assertIn("SomeGun", out)
        self.assertIn("Stagger Offset=`0`", out)
        self.assertNotIn("formid=", out)
    ...
    def test_unwrap_leaves_multi_key_elements_untouched(self):
        ...
        self.assertEqual(change_entries._unwrap_element_wrapper(elem), elem)
...
class TestKeyDictDisplay(unittest.TestCase):
    def test_enum_object_values_render_as_names(self):
        ...
        out = change_entries._key_dict_display(key, {})
    ...
    def test_plain_scalar_key_with_ref_names_annotation(self):
        ...
        out = change_entries._key_dict_display(key, ref_names)
    ...
    def test_empty_or_non_dict_key_falls_back_to_format_scalar(self):
        self.assertEqual(change_entries._key_dict_display({}, {}), pl.format_scalar({}, {}))
        self.assertEqual(change_entries._key_dict_display(5, {}), pl.format_scalar(5, {}))
...
class TestRedundantCounts(unittest.TestCase):
    def test_lvli_count_marked_redundant(self):
        ...
        entries = change_entries.extract_changes(rec["field_changes"], diff_data["ref_names"])
        change_entries.mark_redundant_counts(entries)
    ...
    def test_is_redundant_count_field_helper(self):
        self.assertTrue(change_entries._is_redundant_count_field("Foo / Bar Count", 3, 4, {(3, 4)}))
        self.assertFalse(change_entries._is_redundant_count_field("Foo / Bar Count", 3, 5, {(3, 4)}))
        self.assertFalse(change_entries._is_redundant_count_field("Foo / Bar", 3, 4, {(3, 4)}))
        self.assertFalse(change_entries._is_redundant_count_field("Foo / Bar Count", True, False, {(True, False)}))
...
class TestCommonChanges(unittest.TestCase):
    def _build_records(self, diff_data):
        ...
        for c in diff_data["changed"]:
            fid = c["stub"]["form_id"]
            entries = change_entries.extract_changes(c["field_changes"], diff_data["ref_names"])
            change_entries.mark_redundant_counts(entries)
            records[fid] = {
                "status": "changed",
    ...
    def test_six_npc_records_collapse_into_one_common_change(self):
        ...
        common = change_entries.compute_common_changes(records, threshold=5)
    ...
    def test_aggression_deltas_do_not_collapse(self):
        diff_data = load_fixture("diff_small.json")
        records = self._build_records(diff_data)
        change_entries.compute_common_changes(records, threshold=5)
    ...
    def test_below_threshold_no_grouping(self):
        diff_data = load_fixture("diff_small.json")
        records = self._build_records(diff_data)
        common = change_entries.compute_common_changes(records, threshold=7)
    ...
    def test_ignores_non_changed_status(self):
        ...
        self.assertEqual(change_entries.compute_common_changes(records, threshold=1), [])
