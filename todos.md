...
Outstanding work lives in [GitHub Issues](https://github.com/Mapekz/FO76-Tools/issues).
...
Settled 2026-07-31: the repo is **MIT-licensed**. `LICENSE` at the repo root, all three real
crates (`esm/`, `ba2/`, `esm/bindings/napi`) carry `license = "MIT"`, both `package.json`s do
too, and `deny.toml`'s `licenses.private` no longer ignores our own crates — they're checked
like any other dependency now. `publish = false` stays on all three; that's orthogonal to
licensing, since these are local tools never uploaded to a registry.
