...
//! considered, but clap's `--help` output easily fits inside a default 64KB
//! Linux pipe buffer in one `write(2)`, so the child could finish writing —
//! and exit successfully — before the parent ever got around to closing its
//! end, making that approach non-deterministic.)
...
/// `ba2 --help` piped into a reader that's already gone must die via SIGPIPE
...
fn broken_stdout_pipe_kills_the_process_instead_of_panicking() {
    let binary = std::env::var_os("CARGO_BIN_EXE_ba2").expect("CARGO_BIN_EXE_ba2 not set");

    // `--help` prints clap's usage text to stdout and needs no archive on
    // disk, so this test has no external dependency.
    let output = Command::new(binary)
        ...
        .stderr(Stdio::piped())
        .output()
        .expect("run `ba2 --help` with a pre-closed stdout pipe");
